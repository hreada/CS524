...
We test our actor semantics on an example of tree recursion~\cite{actor}.
The problem is to calculate the product of all leaves of a binary tree.
The problem is solved by a divide-and-conquer algorithm which calculates the 
results for two subtrees and multiplies the results.
The algorithm is implemented using a \emph{join continuation} as follows.
\begin{verbatim}
let(bind(sink,
...
\end{verbatim}
Remind that our semantics initializes the initial actor with the behavior of 
